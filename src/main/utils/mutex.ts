 ...
 * @returns Promise<T> - Result of the function execution
 */
export const withLock = async <T>(resourceName: string, fn: () => Promise<T> | T, timeout?: number): Promise<T> => mutex.withLock(resourceName, fn, timeout);
 ...
 * @returns Promise<() => void> - Release function to unlock the resource
 */
export const acquireLock = async (resourceName: string, timeout?: number): Promise<() => void> => mutex.acquire(resourceName, timeout);
 ...
 * @returns boolean - True if the resource is locked
 */
export const isLocked = (resourceName: string): boolean => mutex.isLocked(resourceName);
